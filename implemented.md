...
Filters shared by both: From/To Date, Store Name, Department, Sub-Department, Vendor Name, Generic Name, Dosage Form, Item. Excel export and a print sheet, both headed with the shop's letterhead (name, address, phone, logo — all set by the Super Admin).

- `GET /stock-report/department-wise` — Outlet Name, Department Name, SKU No, QOH, COGS, Sales Value, Profit (TK), Profit (%), a **Total** per outlet and one **Grand Total**.
- `GET /stock-report/vendor-wise` — Sl, Store Name, Vendor Name, Current Stock, Total Sales Value, Total Cost Value, plus Grand Total; ordered by current stock ascending.
- `GET /stock-report/export?reportType=DEPARTMENT|VENDOR` — the same rows as XLSX.

Every figure comes from current `Batch` rows, not sales history: `QOH = SUM(stockQty)`, `COGS = SUM(stockQty * purchasePrice)`, `Sales Value = SUM(stockQty * sellingPrice)`, `Profit = Sales Value - COGS`, `Profit % = Profit / Sales Value * 100`. Only batches with `stockQty > 0` count — a product with none left is not a SKU on hand, and including it would pad every row with the whole 17k catalog. Profit is derived in one helper used by the rows, the per-outlet Total **and** the Grand Total, so the three can never disagree.

**Outlet Name / Store Name show the shop's own name**, not the internal branch name — a single-branch pharmacy reads "Bismillah Pharmacy" rather than "Main Store". A shop with more than one branch gets the branch appended in parentheses so its rows stay distinguishable (one `store.count` per request decides which).

From/To Date are blank by default so the report reads as a plain "everything on hand" snapshot; when set they narrow it to stock **received** (batch created) inside the window.

Models: `Batch`, `Product`, `Department`, `Store`, `Supplier`.

---

## Financial Overview (whole-shop)

A general-ledger view over what the app already records — nothing is hand-entered, so the numbers cannot disagree with the screens they come from. Fully dynamic: date range, store filter, live charts. **Frontend**: `MediboxFinanceOverviewView.tsx`. **Backend**: `financeRoutes.ts`. Permission: `finance-overview`.

- `GET /finance/overview` — headline totals, the previous comparable period, growth, a month-by-month series, and a whole-shop `snapshot`.
- `GET /finance/journal` — the ledger itself, paged, newest first.

Sources: Income = `Sale.netAmount` (Billing); COGS via the Dashboard's own `aggregateCogs`, so Gross Profit matches there exactly; Purchases = approved `Grn.netAmount`; Expenses = `Expense.amount`; Salaries = `EmployeeSalary.amount` where status is PAID (an UNPAID row is a plan, not a transaction). `Gross Profit = Income - COGS`; `Net Profit = Gross Profit - Expenses - Salaries`. Debit/credit follow retail sense: money in is credit, money out is debit.

The **Shop Snapshot** strip shows what the shop holds as it stands: stock value at cost and at retail, quantity on hand, SKUs in stock, catalog size, customers, suppliers, employees and store count.

Growth compares the immediately preceding window **of the same length**, so a 30-day view is measured against the 30 days before it rather than a calendar boundary. The monthly series emits every month in range including empty ones, so the trend line shows a real gap instead of joining across it, and uses one grouped query per source rather than one per month. The journal is assembled from four tables with a bounded fetch from each and reports `truncated` when a source hits the cap.

Models: `Sale`, `SaleItem`, `Batch`, `Grn`, `Expense`, `EmployeeSalary`, `Product`, `Customer`, `Employee`, `Supplier`, `Store`.
