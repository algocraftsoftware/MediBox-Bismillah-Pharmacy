...
<<<<<<< HEAD
      ...
      -- a placeholder expiry two years out. Neither was ever entered by anyone,
      -- so neither is shown: billing reports them as blank rather than quoting
      -- a batch and an expiry the carton does not carry.
...
<<<<<<< HEAD
  ...
  const isFallback = !!batch.product.externalCode
...
//    dip under the percentage floor.
//
//    What the 9% non-pharma floor allows, since the instruction is written in
//    terms of the discount rather than the margin left behind. With a baseline
//    margin of g% and a discount of d% off the selling price, the margin that
//    survives is (g - d) / (100 - d):
//      g = 14%  → up to  5% off (leaves 9.5%); 6% would leave 8.5% and is refused
//      g = 20%  → up to 12% off, so the stated 10% is comfortably allowed
//      g = 40%  → up to 34% off, so the stated 30% is allowed
//      g = 50%  → up to 45% off
...
<<<<<<< HEAD
          ...
          // line still references the batch row itself through batchId.
          batchNoSnapshot: batch.product.externalCode
            && batch.batchNo === fallbackBatchNo(batch.product.externalCode)
            ? ''
            : batch.batchNo,
=======
...
<<<<<<< HEAD
        ...
        // that day against an older sale.
        await tx.duePayment.create({
          data: {
            ...
            paidMobileBanking: Number(paidMobileBanking) || 0,
            paidCard: Number(paidCard) || 0,
            collectedById: req.auth!.sub as number,
          },
        });
