...
router.post('/:id/unapprove', requireAdminRole, asyncHandler(async (req, res) => {
  ...
  const existing = await prisma.vst.findFirst({
    where: { id, shopId },
    include: { items: { include: { product: { select: { name: true } } } } },
  });
  if (!existing) return res.status(404).json({ error: 'VST not found' });
  if (existing.status !== 'APPROVED') {
    return res.status(400).json({ error: 'Only an approved VST can be un-approved' });
  }

  // An RTV is raised against a VST's lines. Unwinding the VST underneath one
  // would leave the RTV pointing at a transfer that never happened.
  const rtvs = await prisma.rtv.count({ where: { vstId: id } });
  if (rtvs > 0) {
    return res.status(400).json({
      error: 'Cannot un-approve: an RTV has been raised against this VST. Remove that RTV first.',
    });
  }
  ...
  try {
    const updated = await prisma.$transaction(
      async (tx) => {
        await restoreIssuedStock(tx, existing.storeId, existing.items);
        return tx.vst.update({
          where: { id },
