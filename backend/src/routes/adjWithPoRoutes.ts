...
router.get('/purchase-orders', asyncHandler(async (req, res) => {
  ...
  // an adjustment of its own is spoken for, and offering it again invites a
  // second document against the same goods.
  //
  // Nothing here is scoped by kind: unlike the GRN screen, an adjustment must
...
router.post('/:id/approve', asyncHandler(async (req, res) => {
  ...
  // through a GRN, so it follows the same rule rather than a stricter one.
  const auth = req.auth!;
...
router.post('/:id/unapprove', requireAdminRole, asyncHandler(async (req, res) => {
  ...
  if (!existing) return res.status(404).json({ error: 'Adjustment not found' });
  if (existing.status !== 'APPROVED') {
    return res.status(400).json({ error: 'Only an approved adjustment can be un-approved' });
  }

  // The RTV adjustment lines stay exactly where they are. They are part of the
  // draft — written when the document is created or edited, not at approval —
  // and the RTV's adjustable balance counts them from that moment, so deleting
  // them here would erase content the admin is un-approving in order to edit.
  // Approval only moved stock and set the status; this undoes precisely that.
  try {
    const updated = await prisma.$transaction(
      async (tx) => {
...
// Attach the supplier's document for this adjustment — a photo, a scan, a PDF
// or an office document. Stored as a URL on the record, which the Invoice
// button on the list opens.
router.post('/:id/attachment', handleUpload(uploadAttachment.single('file')), asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return res.status(400).json({ error: 'Invalid adjustment id' });
  if (!req.file) return res.status(400).json({ error: 'Choose a file to attach' });

  // kind pins this to an Adjust With PO — a standard GRN With PO shares the
  // table and must not be reachable through this route.
  const existing = await prisma.grn.findFirst({
    where: { id, shopId: req.shop!.id, kind: 'ADJUST_WITH_PO' },
  });
  if (!existing) return res.status(404).json({ error: 'Adjustment not found' });

  // A file the storage service refuses is a problem with what was chosen, not
  // with the server, so it is reported as such rather than surfacing as a 500.
