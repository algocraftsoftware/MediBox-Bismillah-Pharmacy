...
<<<<<<< HEAD
    LEFT JOIN LATERAL (
      ...
      -- store (an everyday occurrence — different batch numbers and expiry
      -- dates) came back as two identical-looking lines in the requisition
      -- grid, and inflated the record count by the same amount.
      --
      -- Stock is the store's whole holding for the product, which is what QOH
      -- means everywhere else in the app; pricing comes from the store's most
      -- recently received batch, the same "latest wins" rule the shop-wide
      -- fallback below already uses.
      SELECT COALESCE(SUM(b2."stockQty"), 0)::int AS "stockQty",
             (ARRAY_AGG(b2."purchasePrice" ORDER BY b2."createdAt" DESC))[1] AS "purchasePrice",
             (ARRAY_AGG(b2.mrp ORDER BY b2."createdAt" DESC))[1] AS mrp
      FROM "Batch" b2
...
// Un-approve a requisition: APPROVED back to UNAPPROVED, so it can be edited
// again. Admin-only.
//
// A requisition that has already been final-approved has become a purchase
// order, and is un-approved from that screen instead — otherwise the order
// number and its GRNs would be left hanging off a draft.
router.post('/:id/unapprove', requireAdminRole, asyncHandler(async (req, res) => {
  ...
  if (existing.status === 'FINAL_APPROVED') {
    return res.status(400).json({
      error: 'This requisition is now a purchase order. Un-approve it from Purchase Order first.',
    });
  }
  if (existing.status !== 'APPROVED') {
    return res.status(400).json({ error: 'Only an approved requisition can be un-approved' });
  }
