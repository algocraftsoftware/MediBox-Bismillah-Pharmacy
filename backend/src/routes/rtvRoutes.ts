...
router.post('/:id/unapprove', requireAdminRole, asyncHandler(async (req, res) => {
  ...
  if (!existing) return res.status(404).json({ error: 'RTV not found' });
  if (existing.status !== 'APPROVED') {
    return res.status(400).json({ error: 'Only an approved RTV can be un-approved' });
  }

  // Its value may already have been set against a GRN or an Adjustment Others.
  const [grnUsed, othersUsed] = await Promise.all([
    prisma.grnRtvAdjustment.count({ where: { rtvId: id } }),
    prisma.adjOthersItem.count({ where: { rtvId: id } }),
  ]);
  if (grnUsed + othersUsed > 0) {
    return res.status(400).json({
      error: 'Cannot un-approve: this RTV has already been adjusted against a GRN or an Adjustment. Remove those adjustments first.',
    });
  }
