...
// proxy's address for every request and the rate limiters bucket the entire
// platform together — see middleware/rateLimit.ts. The value is 1, not `true`:
// exactly one proxy hop is trusted, so a client cannot spoof its own address by
// sending an X-Forwarded-For header of its own (express-rate-limit also refuses
