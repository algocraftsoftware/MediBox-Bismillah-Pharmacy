...
export const MediboxCustomerRegistrationView: React.FC = () => {
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-700 font-bold uppercase whitespace-nowrap">
              ...
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">CUSTOMER ID</th>
              <th className="py-4 px-3 border border-slate-300 w-[12%] whitespace-nowrap">CUSTOMER NAME</th>
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">TYPE</th>
              <th className="py-4 px-3 border border-slate-300 w-[11%] whitespace-nowrap">PHONE NUMBER</th>
              <th className="py-4 px-3 border border-slate-300 w-[10%] whitespace-nowrap">ADDRESS</th>
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">BIRTH DATE</th>
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">ENTRY DATE</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[10%] whitespace-nowrap">CREDIT LIMIT</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">REWARD</th>
              <th className="py-4 px-3 border border-slate-300 text-center w-[8%] whitespace-nowrap">EDIT</th>
            </tr>
          </thead>
          <tbody className="font-medium whitespace-nowrap">
            {pageCustomers.map((c) => (
              <tr key={c.id} className="odd:bg-white even:bg-slate-50 hover:bg-emerald-50/50 transition-colors">
                <td className="py-4 px-3 border border-slate-200 font-semibold text-slate-700 whitespace-nowrap">{c.store?.name}</td>
                <td className="py-4 px-3 border border-slate-200 font-bold text-emerald-800 whitespace-nowrap">{c.customerCode}</td>
                <td className="py-4 px-3 border border-slate-200 font-bold text-slate-900 whitespace-nowrap">{c.name}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{c.custType === "VVIP" ? "VVIP CUSTOMER" : c.custType}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-700 whitespace-nowrap">{c.mobile}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{c.address || "—"}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-500 whitespace-nowrap">{c.birthDate ? c.birthDate.split("T")[0] : "N/A"}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{new Date(c.createdAt).toLocaleDateString()}</td>
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{c.creditLimit.toFixed(2)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{c.rewardBalance.toFixed(2)}</td>
