...
export const DetailView: React.FC<{
  ...
  const { shopName, logoUrl, adminName, adminRole } = useShopSession();
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap sticky top-0">
              ...
              <th className="py-4 px-3 border border-slate-300 text-right w-[5%] whitespace-nowrap">Total Value</th>
          ...
          <tbody className="font-medium whitespace-nowrap">
            {computed.map((it, idx) => (
              <tr key={it.productId} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                ...
                <td className="py-1.5 px-3 text-right font-bold text-slate-900 whitespace-nowrap">{fmt(it.unitPrice)}</td>
                ...
                <td className="py-1.5 px-3 text-right text-slate-600 whitespace-nowrap">{fmt(it.gp)}</td>
                <td className="py-1.5 px-3 text-right text-slate-600 whitespace-nowrap">{it.gpPct.toFixed(2)}</td>
                <td className="py-1 px-2 whitespace-nowrap">
                  <input
                    value={it.batchNo}
                    disabled={isLocked}
                ...
                <td className="py-1.5 px-3 text-right font-bold text-slate-900 whitespace-nowrap">{fmt(it.netTotal)}</td>
      ...
      <div className="bg-white border-t border-slate-300 p-3 shrink-0 text-xs flex items-start gap-4">
        <div className="flex-1 space-y-2">
          ...
          <div className="flex items-end gap-2.5">
            ...
            <GrnAttachment
              ...
              disabled={isLocked}
              onUpload={(file) => api.uploadGrnwAttachment(grn.id, file)}
              onUploaded={(result) => {
        ...
        <div className="flex flex-col justify-end gap-1.5 shrink-0">
          ...
          <UnapproveButton
            ...
            movesStock
            onUnapprove={() => api.unapproveGrnw(grn.id)}
            onDone={(result) => {
              setGrn(result as typeof grn);
