...
import { fmt, upper } from "../../../lib/format";
...
export function buildPoReportHtml(params: {
  ...
  const itemRows = items
    ...
    .join("");
  return `
    <html><head><title>${upper(order.orderNo) || "PURCHASE ORDER"}</title>
    <style>
    ...
    <div class="meta">
      <div class="row"><span>Vendor Name</span><span>${upper(order.supplier?.name)}</span></div>
      <div class="row"><span>Order No</span><span>${upper(order.orderNo)}</span></div>
