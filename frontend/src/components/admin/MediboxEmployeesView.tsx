...
export const MediboxEmployeesView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] text-slate-900">
      ...
      <div className="bg-white border border-slate-300 rounded-xl shadow-sm overflow-hidden">
        {filtered.length === 0 ? (
          <div className="overflow-x-auto p-10 text-center text-slate-400 font-bold">
          ...
          <table className="w-full text-sm border border-slate-300">
            <thead>
              <tr className="bg-slate-200/90 text-slate-800 font-black uppercase whitespace-nowrap">
                <th className="px-4 py-4 border border-slate-300 w-[12%] whitespace-nowrap">Name</th>
                <th className="px-4 py-4 border border-slate-300 w-[12%] whitespace-nowrap">Mobile</th>
                <th className="px-4 py-4 border border-slate-300 w-[20%] whitespace-nowrap">Address</th>
                <th className="px-4 py-4 border border-slate-300 w-[6%] whitespace-nowrap">Age</th>
                <th className="px-4 py-4 border border-slate-300 w-[14%] whitespace-nowrap">Education</th>
                <th className="px-4 py-4 border border-slate-300 text-right w-[14%] whitespace-nowrap">Salary (৳)</th>
                <th className="px-4 py-4 border border-slate-300 text-right w-[22%] whitespace-nowrap">Actions</th>
              </tr>
            </thead>
            <tbody className="whitespace-nowrap">
              {pageRows.map((e) => (
                <tr key={e.id} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                  <td className="px-4 py-4 border border-slate-200 font-bold text-slate-800 whitespace-nowrap">{e.name}</td>
                  <td className="px-4 py-4 border border-slate-200 font-semibold text-slate-600 whitespace-nowrap">{e.mobile}</td>
                  <td className="px-4 py-4 border border-slate-200 font-semibold text-slate-600 whitespace-nowrap">{e.address || "—"}</td>
                  <td className="px-4 py-4 border border-slate-200 font-semibold text-slate-600 whitespace-nowrap">{e.age ?? "—"}</td>
                  <td className="px-4 py-4 border border-slate-200 font-semibold text-slate-600 whitespace-nowrap">{e.education || "—"}</td>
                  <td className="px-4 py-4 border border-slate-200 font-bold text-slate-800 text-right whitespace-nowrap">{e.salary.toLocaleString()}</td>
                  <td className="px-4 py-4 border border-slate-200 text-right whitespace-nowrap">
                    <button
                      onClick={() => openEdit(e)}
