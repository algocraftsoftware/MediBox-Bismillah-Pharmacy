...
import { CreatedStockRow } from "../../types";
...
// Every field on this form is mandatory, so an empty one is marked as soon as
// a publish has been attempted — the banner says something is missing, this
// says which.
const invalidInputClass =
  "w-full border border-red-500 bg-red-50 rounded px-2 py-1.5 font-semibold outline-none focus:border-red-600";

// The one message the form shows when anything at all is still blank.
const INCOMPLETE_MESSAGE = "Please fulfill all information";

// Item Type is the only optional entry — everything below has to be filled in
// before an item can be published.
type RequiredField =
  | "warehouseId"
  | "itemName"
  | "genericName"
...
export const MediboxCreateStockView: React.FC = () => {
  ...
  const isMissing = (f: RequiredField) => missing.has(f);
  const fieldClass = (f: RequiredField) => (isMissing(f) ? invalidInputClass : inputClass);
  ...
  const handlePublish = async () => {
    // Nothing is optional but Item Type: an item published half-filled reaches
    // Billing, GRN and every report with holes in it, so the form refuses the
    // whole entry rather than accepting part of one.
    const blank: RequiredField[] = [];
    if (!warehouseId) blank.push("warehouseId");
    if (!itemName.trim()) blank.push("itemName");
    if (!genericName.trim()) blank.push("genericName");
    if (!displayCategory.trim()) blank.push("displayCategory");
    if (!departmentId) blank.push("departmentId");
    if (!subDepartmentId) blank.push("subDepartmentId");
    if (!supplierId) blank.push("supplierId");
    if (!unit.trim()) blank.push("unit");
    if (!reorderLevel.trim()) blank.push("reorderLevel");
    if (!boxQty.trim()) blank.push("boxQty");
    if (!purchasePrice.trim()) blank.push("purchasePrice");
    if (!salesPrice.trim()) blank.push("salesPrice");
    ...
    try {
      const row = await api.createStock({
        ...
        displayCategory: displayCategory.trim(),
        ...
        <div className="max-w-6xl mx-auto space-y-4 text-xs">
          <div className="bg-white border border-slate-300 rounded-lg shadow-sm">
            ...
            <div className="p-4 space-y-3">
              {/* Sits above the fields, not beside the buttons, so it is the
                  first thing seen after a refused publish. */}
              {error && (
                <div className="bg-red-600 text-white font-bold rounded px-4 py-2.5" role="alert">
                  {error}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                <SearchableSelect
                  ...
                  placeholder="Type or pick..."
                  allowFreeText
                  invalid={isMissing("displayCategory")}
                ...
                <SearchableSelect
                  ...
                  placeholder="Select..."
                  invalid={isMissing("departmentId")}
                ...
                <SearchableSelect
                  ...
                  invalid={isMissing("subDepartmentId")}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 items-end">
                ...
                <Field label="Re-order Level*">
                  <input
                    ...
                    className={`${fieldClass("reorderLevel")} text-right`}
                  />
                </Field>
                <Field label="Box Qty*">
                  <input
                    ...
                    className={`${fieldClass("boxQty")} text-right`}
                  />
                </Field>
                <Field label="Purchase Price*">
                  <input
                    ...
                    placeholder="0.00"
                    className={`${fieldClass("purchasePrice")} text-right`}
                  />
                </Field>
                <Field label="Sales Price*">
                  <input
                    ...
                    placeholder="0.00"
                    className={`${fieldClass("salesPrice")} text-right`}
                  />
                </Field>
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <span className="text-[11px] font-semibold text-slate-500">
                  Every field except Item Type is required. Purchase Price and Sales Price are recorded against the
                  selected warehouse. Stock Qty always starts at 0 — receive quantities through GRN With PO or GRN
                  Without PO.
                </span>
              </div>
            </div>
