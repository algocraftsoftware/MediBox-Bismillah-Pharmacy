...
export const MediboxExpensesView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] text-slate-900">
      ...
      <div className="bg-white border border-slate-300 rounded-xl shadow-sm overflow-hidden">
        {rows.length === 0 ? (
          <div className="overflow-x-auto p-10 text-center text-slate-400 font-bold">No expenses recorded for this period.</div>
        ) : (
          <table className="w-full text-sm border border-slate-300">
            <thead>
              <tr className="bg-slate-200/90 text-slate-800 font-black uppercase whitespace-nowrap">
                <th className="px-4 py-4 border border-slate-300 w-[6%] whitespace-nowrap">Sl</th>
                <th className="px-4 py-4 border border-slate-300 w-[28%] whitespace-nowrap">Expense Name</th>
                <th className="px-4 py-4 border border-slate-300 text-right w-[14%] whitespace-nowrap">Price (৳)</th>
                <th className="px-4 py-4 border border-slate-300 w-[16%] whitespace-nowrap">Added By</th>
                <th className="px-4 py-4 border border-slate-300 w-[16%] whitespace-nowrap">Date</th>
                <th className="px-4 py-4 border border-slate-300 text-right w-[20%] whitespace-nowrap">Actions</th>
              </tr>
            </thead>
              ...
              {pageRows.map((r, idx) => {
                ...
                return (
                  <tr key={r.id} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                    <td className="px-4 py-4 border border-slate-200 text-slate-500 whitespace-nowrap">{(page - 1) * PAGE_SIZE + idx + 1}</td>
                    <td className="px-4 py-4 border border-slate-200 font-bold text-slate-800 whitespace-nowrap">
                      {isEditing ? (
                        <input
                          value={editName}
                    ...
                    <td className="px-4 py-4 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">
                      {isEditing ? (
                        <input
                          type="number"
                          min="0"
                    ...
                    <td className="px-4 py-4 border border-slate-200 font-semibold text-slate-600 whitespace-nowrap">{r.createdBy?.name}</td>
                    <td className="px-4 py-4 border border-slate-200 font-semibold text-slate-600 whitespace-nowrap">{new Date(r.createdAt).toLocaleString()}</td>
