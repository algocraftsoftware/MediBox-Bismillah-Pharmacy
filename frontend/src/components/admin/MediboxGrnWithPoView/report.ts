...
// SUBMIT) and the List page's PDF button (fed from the persisted Grn record),
// so the two always render identical output. Mirrors the arrangement GRN
// Without PO already uses.
// =======================================================
...
export interface GrnReportTotals {
...
export function buildGrnReportHtml(params: {
  grn: Grn;
  shopName: string | null;
  logoUrl: string | null;
  items: GrnReportItem[];
  totals: GrnReportTotals;
  invoiceDiscount: number;
  invoiceVat: number;
  expiryAdjustmentAmount: number;
}): string {
  const { grn, shopName, logoUrl, items, totals, invoiceDiscount, invoiceVat, expiryAdjustmentAmount } = params;
  ...
  const itemRows = items
    ...
    .join("");
  return `
    <html><head><title>${upper(grn.transactionNo)}</title>
    <style>
    ...
    <div class="meta">
      <div class="row"><span>GRN No.</span><span>${upper(grn.transactionNo)}</span></div>
      <div class="row"><span>Status</span><span>${statusLabel(grn.status)}</span></div>
      <div class="row"><span>Order No.</span><span>${upper(grn.purchaseOrder?.orderNo) || "—"}</span></div>
      <div class="row"><span>Invoice No.</span><span>${upper(grn.invoiceNo)}</span></div>
      ...
      <div class="row"><span>Supplier</span><span>${upper(grn.supplier?.name)}</span></div>
