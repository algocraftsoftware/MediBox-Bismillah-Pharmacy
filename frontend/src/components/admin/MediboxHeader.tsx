...
import { ADMIN_ONLY_MENU_FEATURES, MENU_FEATURES } from "../../lib/menuFeatures";
import { ShopLogoBadge } from "./ShopLogoBadge";

interface MediboxHeaderProps {
  shopSlug: string;
  activeRoute: string;
  shopName: string;
  logoUrl: string | null;
  adminName: string;
  // `permissions` alone decides which of the grantable features show, so a role
  // can never quietly re-open one that was switched off. adminRole is used only
  // for ADMIN_ONLY_MENU_FEATURES, which are not grantable in the first place.
  permissions: string[];
...
export const MediboxHeader: React.FC<MediboxHeaderProps> = ({
  shopSlug,
  activeRoute,
  shopName,
  logoUrl,
  ...
  const visibleItems = [
    ...MENU_FEATURES.filter((item) => permissions.includes(item.id)),
    ...(adminRole === "ADMIN" ? ADMIN_ONLY_MENU_FEATURES : []),
  ];
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute left-0 top-full mt-2 min-w-140 w-max max-w-[95vw] bg-white text-slate-800 rounded-lg shadow-2xl border border-slate-200 p-6 z-50">
                ...
                <div className="mt-5 pt-3 border-t border-slate-200 text-xs text-slate-400 font-semibold">
                  <span className="normal-case">
                    MediBox Powered by{" "}
                    {/* Opens in its own tab so a click never drops the user out
                        of the app mid-task; rel guards the new tab against
                        reaching back through window.opener. */}
                    <a
                      href="https://algocraftsoftware.com"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-slate-400 hover:text-[#047857] underline underline-offset-2 transition-colors"
                    >
