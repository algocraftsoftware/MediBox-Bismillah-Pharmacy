  ...
  const handleRowReport = async (id: number) => {
    ...
    try {
      ...
      const itemRows = (vst.items || [])
        ...
        .join("");
      setReportHtml(`
        <html><head><title>${upper(vst.vstNo)}</title>
        <style>
        ...
        <div class="meta">
          <p><b>Store Name:</b> ${vst.store?.name || ""} &nbsp; <b>VST No:</b> ${upper(vst.vstNo)} &nbsp; <b>VST Date &amp; Time:</b> ${new Date(vst.createdAt).toLocaleString()}</p>
          <p><b>Supplier Name:</b> ${upper(vst.supplier?.name)} &nbsp; <b>Status:</b> ${statusLabel(vst.status)} &nbsp; <b>Approved By:</b> ${vst.approvedBy?.name || "—"}</p>
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap sticky top-0">
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">VST From</th>
              ...
              <th className="py-4 px-3 border border-slate-300 text-right w-[8%] whitespace-nowrap">VST Qty (SKU)</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">VST Value</th>
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">VST By</th>
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">VST Date</th>
              ...
              <th className="py-4 px-3 border border-slate-300 text-right w-[8%] whitespace-nowrap">VST to RTV(%)</th>
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">Remarks</th>
              <th className="py-4 px-3 border border-slate-300 text-center w-[7%] whitespace-nowrap"></th>
            </tr>
          </thead>
          <tbody className="font-medium whitespace-nowrap">
            {rows.map((r) => (
              <tr key={r.id} className="odd:bg-white even:bg-slate-50 hover:bg-emerald-50/50">
                ...
                <td className="py-4 px-3 border border-slate-200 font-bold text-slate-900 whitespace-nowrap">{r.vstNo}</td>
                <td className="py-4 px-3 border border-slate-200 text-right whitespace-nowrap">{r.itemCount ?? 0}</td>
                <td className="py-4 px-3 border border-slate-200 text-right whitespace-nowrap">{fmt(r.totalAmount)}</td>
                ...
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{(r.vstToRtvPct ?? 0).toFixed(2)}</td>
