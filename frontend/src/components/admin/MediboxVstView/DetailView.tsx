...
import { Vst, VstSearchItemRow } from "../../../types";
...
export const DetailView: React.FC<{
  ...
  const handleReport = () => {
    ...
    const itemRows = computed
      ...
      .join("");
    setReportHtml(`
      <html><head><title>${upper(vst?.vstNo) || "VST"}</title>
      <style>
      ...
      <div class="meta">
        <p><b>Store Name:</b> ${storeName} &nbsp; <b>VST No:</b> ${upper(vst?.vstNo)} &nbsp; <b>VST Date &amp; Time:</b> ${vst ? new Date(vst.createdAt).toLocaleString() : ""}</p>
        <p><b>Supplier Name:</b> ${upper(supplierName)} &nbsp; <b>Status:</b> ${vst ? statusLabel(vst.status) : "Unapproved"} &nbsp; <b>Approved By:</b> ${vst?.approvedBy?.name || "—"}</p>
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap sticky top-0">
              ...
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">Item No</th>
              <th className="py-4 px-3 border border-slate-300 w-[12%] whitespace-nowrap">Item Name</th>
              ...
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">MRP/PCs</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[8%] whitespace-nowrap">Existing QOH</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[8%] whitespace-nowrap">VST Qty (PCs)</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[9%] whitespace-nowrap">Total PP Value</th>
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">Batch</th>
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">Exp.Date</th>
              <th className="py-4 px-3 border border-slate-300 w-[8%] whitespace-nowrap">Remarks</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">Final QOH</th>
          ...
          <tbody className="font-medium whitespace-nowrap">
            {computed.map((it, idx) => (
              <tr key={`${it.productId}-${it.batchNo}`} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                ...
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{fmt(it.ppPerPiece)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{fmt(it.mrpPerPiece)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{it.existingQoh}</td>
          <td className="py-2 px-2 border border-slate-200 text-right whitespace-nowrap">
            ...
            />
                ...
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{fmt(it.totalPpValue)}</td>
                ...
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{it.finalQoh}</td>
      ...
      <div className="bg-white border-t border-slate-300 p-3 shrink-0 text-xs flex items-center gap-4">
        ...
        <div className="flex gap-1.5 shrink-0">
          ...
          <UnapproveButton
            ...
            label={`VST ${vst?.vstNo}`}
            movesStock
            onUnapprove={() => api.unapproveVst(vst!.id)}
            onDone={(result) => {
              setVst(result as typeof vst);
