...
import { ExpireProductsType, ExpireProductRow } from "../../types";
...
export const MediboxExpireProductsView: React.FC = () => {
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap sticky top-0">
              ...
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">Item No</th>
              <th className="py-4 px-3 border border-slate-300 w-[20%] whitespace-nowrap">Item Name</th>
              <th className="py-4 px-3 border border-slate-300 w-[8%] whitespace-nowrap">Group</th>
              <th className="py-4 px-3 border border-slate-300 w-[17%] whitespace-nowrap">Supplier</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">MRP</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[6%] whitespace-nowrap">PP</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[9%] whitespace-nowrap">Stock Qty</th>
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">Batch</th>
              <th className="py-4 px-3 border border-slate-300 w-[10%] whitespace-nowrap">Exp.Date</th>
            </tr>
          </thead>
          <tbody className="font-medium whitespace-nowrap">
            {rows.map((r, i) => (
              <tr key={`${r.itemNo}-${r.batch}-${i}`} className="odd:bg-white even:bg-slate-50 hover:bg-emerald-50/50">
                <td className="py-4 px-3 border border-slate-200 text-slate-500 whitespace-nowrap">{(page - 1) * PAGE_SIZE + i + 1}</td>
                <td className="py-4 px-3 border border-slate-200 font-bold text-slate-800 whitespace-nowrap">{r.itemNo}</td>
                <td className="py-4 px-3 border border-slate-200 font-bold text-slate-900 whitespace-nowrap">{r.itemName}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{r.group || "—"}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{r.supplier || "—"}</td>
                <td className="py-4 px-3 border border-slate-200 text-right whitespace-nowrap">{r.mrp.toFixed(2)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right whitespace-nowrap">{r.pp.toFixed(2)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{r.stockQty}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{r.batch}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{r.expDate.split("T")[0]}</td>
              </tr>
            ))}
