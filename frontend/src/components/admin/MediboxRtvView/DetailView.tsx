...
import { Rtv, RtvVia, VstOption } from "../../../types";
...
export const DetailView: React.FC<{
  ...
  const handleReport = () => {
    ...
    const itemRows = computed
      ...
      .join("");
    setReportHtml(`
      <html><head><title>${upper(rtv?.rtvNo) || "RTV"}</title>
      <style>
      ...
      <div class="meta">
        <p><b>From Store:</b> ${storeName} &nbsp; <b>RTV VIA:</b> ${viaLabel(via)} &nbsp; <b>RTV No:</b> ${upper(rtv?.rtvNo)} &nbsp; <b>RTV Date:</b> ${rtv ? new Date(rtv.createdAt).toLocaleString() : ""}</p>
        <p><b>Supplier:</b> ${upper(supplierName)} &nbsp; <b>VST No:</b> ${upper(vstOptions.find((o) => String(o.id) === vstId)?.vstNo || rtv?.vst?.vstNo)} &nbsp; <b>Status:</b> ${rtv ? statusLabel(rtv.status) : "Unapproved"}</p>
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap sticky top-0">
              ...
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">Item No</th>
              <th className="py-4 px-3 border border-slate-300 w-[10%] whitespace-nowrap">Item Name</th>
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">Group</th>
              <th className="py-4 px-3 border border-slate-300 w-[5%] whitespace-nowrap">UOM</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">Pack Size</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[8%] whitespace-nowrap">Purchase Price</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[8%] whitespace-nowrap">Sales Price</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[6%] whitespace-nowrap">Item Qty</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[6%] whitespace-nowrap">RTV Qty</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">RTV Value</th>
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">Exp.Date</th>
              <th className="py-4 px-3 border border-slate-300 w-[5%] whitespace-nowrap">Batch</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[7%] whitespace-nowrap">Rem.Value</th>
              <th className="py-4 px-3 border border-slate-300 text-center w-[4%] whitespace-nowrap">
                <input type="checkbox" checked={allSelected} onChange={toggleAll} disabled={isLocked} />
              </th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[5%] whitespace-nowrap">QOH</th>
            </tr>
          </thead>
          <tbody className="font-medium whitespace-nowrap">
            {computed.map((it, idx) => (
              <tr key={it.vstItemId} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                ...
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{it.dosageForm || "—"}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{it.uom}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{it.packSize}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{fmt(it.purchasePrice)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{fmt(it.salesPrice)}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{it.itemQtyPieces}</td>
                ...
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{fmt(it.rtvValue)}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{it.expiryDate}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{it.batchNo}</td>
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{fmt(it.remainingValue)}</td>
                ...
                <td className="py-4 px-3 border border-slate-200 text-right text-slate-600 whitespace-nowrap">{it.remainingQty}</td>
              </tr>
            ))}
            {computed.length === 0 && (
              <tr>
      ...
      <div className="bg-white border-t border-slate-300 p-3 shrink-0 text-xs flex items-center gap-4">
        ...
        <div className="flex gap-1.5 shrink-0">
          ...
          <UnapproveButton
            status={rtv?.status}
            label={`RTV ${rtv?.rtvNo}`}
            onUnapprove={() => api.unapproveRtv(rtv!.id)}
            onDone={(result) => {
              setRtv(result as typeof rtv);
