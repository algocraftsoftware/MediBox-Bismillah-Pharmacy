...
import { AdjOthers, AdjOthersType, RtvAdjustOption, RtvVia } from "../../../types";
    ...
    <div className="flex flex-col h-[calc(100vh-3.25rem)] bg-[#f8fafc] overflow-hidden">
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap">
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">SL</th>
              <th className="py-4 px-3 border border-slate-300 w-[17%] whitespace-nowrap">RTV No</th>
              <th className="py-4 px-3 border border-slate-300 w-[16%] whitespace-nowrap">RTV Date</th>
              <th className="py-4 px-3 border border-slate-300 w-[24%] whitespace-nowrap">Store Name</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[15%] whitespace-nowrap">RTV Amt</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[14%] whitespace-nowrap">ADJ Amt</th>
              <th className="py-4 px-3 border border-slate-300 text-center w-[8%] whitespace-nowrap"></th>
            </tr>
          </thead>
          <tbody className="font-medium whitespace-nowrap">
            {rtvLines.map((l, idx) => (
              <tr key={l.rtvId} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100">
                ...
                <td className="py-4 px-3 border border-slate-200 font-bold text-slate-900 whitespace-nowrap">{l.rtvNo}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{l.rtvDate}</td>
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{l.storeName}</td>
                <td className="py-4 px-3 border border-slate-200 text-right whitespace-nowrap">{fmt(l.rtvAmount)}</td>
      ...
      <div className="bg-white border-t border-slate-300 p-3 shrink-0 text-xs flex items-start gap-4">
        <div className="flex-1 flex items-end gap-2.5">
          {/* The button used to be a plain <button> with nothing behind it,
              so clicking it did nothing at all. */}
          <GrnAttachment
            docLabel="adjustment"
            attachmentUrl={adj?.attachmentUrl}
            disabled={isLocked}
            onUpload={adj ? (file) => api.uploadAdjOthersAttachment(adj.id, file) : undefined}
            onUploaded={(result) => {
        ...
        <div className="flex gap-1.5 shrink-0">
          ...
          <UnapproveButton
            status={adj?.status}
            label={`Adjustment ${adj?.txnNo}`}
            onUnapprove={() => api.unapproveAdjOthers(adj!.id)}
            onDone={(result) => {
              setAdj(result as typeof adj);
