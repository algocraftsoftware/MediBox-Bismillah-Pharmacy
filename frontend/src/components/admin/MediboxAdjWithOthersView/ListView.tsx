...
import { buildAdjOthersReportHtml } from "./report";
...
export const ListView: React.FC<{
  ...
  const handlePdf = async (id: number) => {
    ...
    setError(null);
    try {
      const a = await api.getAdjOthers(id);
      setReportHtml(
        buildAdjOthersReportHtml({
          txnNo: a.txnNo,
          ...
          adjTypeText: adjTypeLabel(a.adjType),
          viaText: viaLabel(a.via),
          transactionDate: new Date(a.createdAt).toLocaleString(),
          approvedAt: a.approvedAt ? new Date(a.approvedAt).toLocaleString() : null,
          remarks: a.remarks,
          shopName,
          logoUrl,
          rtvLines: (a.items || []).map((l) => ({
            rtvNo: l.rtv.rtvNo,
            rtvDate: new Date(l.rtv.createdAt).toLocaleDateString(),
            storeName: l.rtv.store?.name || "",
            rtvAmount: l.rtv.totalAmount,
            adjustmentAmount: l.adjustmentAmount,
          })),
          totalAdjustmentAmount: a.totalAdjustmentAmount,
        ...
        <table className="w-full text-left border-collapse text-sm border border-slate-300">
          <thead>
            <tr className="bg-slate-200/90 text-slate-800 font-bold uppercase whitespace-nowrap sticky top-0">
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">Trans No</th>
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">ADJ.Store</th>
              <th className="py-4 px-3 border border-slate-300 w-[9%] whitespace-nowrap">Supplier</th>
              <th className="py-4 px-3 border border-slate-300 w-[10%] whitespace-nowrap">Adjust.Type</th>
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">RTV VIA</th>
              <th className="py-4 px-3 border border-slate-300 w-[7%] whitespace-nowrap">Status</th>
              <th className="py-4 px-3 border border-slate-300 w-[6%] whitespace-nowrap">User</th>
              <th className="py-4 px-3 border border-slate-300 w-[10%] whitespace-nowrap">Approved By</th>
              <th className="py-4 px-3 border border-slate-300 w-[10%] whitespace-nowrap">Approve Date</th>
              <th className="py-4 px-3 border border-slate-300 text-right w-[17%] whitespace-nowrap">Total Adjustment Amt</th>
          ...
          <tbody className="font-medium whitespace-nowrap">
            {rows.map((r) => (
              <tr key={r.id} className="odd:bg-white even:bg-slate-50 hover:bg-emerald-50/50">
                <td className="py-4 px-3 border border-slate-200 font-bold text-slate-900 whitespace-nowrap">{r.txnNo}</td>
                ...
                <td className="py-4 px-3 border border-slate-200 text-slate-600 whitespace-nowrap">{adjTypeLabel(r.adjType)}</td>
                ...
                <td className="py-4 px-3 border border-slate-200 text-right font-bold text-slate-900 whitespace-nowrap">{fmt(r.totalAdjustmentAmount)}</td>
