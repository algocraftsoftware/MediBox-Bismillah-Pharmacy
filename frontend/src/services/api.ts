...
<<<<<<< HEAD
  ...
  listContacts: (
    token: string,
    params?: { search?: string; shopId?: string | number; page?: number; pageSize?: number },
  ) => {
    ...
    return request<ContactListResponse>(`/superadmin/contacts${qsStr ? "?" + qsStr : ""}`, token);
  },

  // Downloads the whole filtered list, not just the page on screen.
  exportContacts: async (token: string, params?: { search?: string; shopId?: string | number }) => {
    const qs = new URLSearchParams();
    Object.entries(params || {}).forEach(([k, v]) => {
      if (v !== undefined && v !== "") qs.set(k, String(v));
    });
    const res = await fetch(`${API_BASE_URL}/superadmin/contacts/export?${qs.toString()}`, {
